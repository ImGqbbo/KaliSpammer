...
namespace KaliSpammer
{
    public class KaliAPI
    {
        ...
        public KaliAPI(string name, string ownerid, string secret, string version)
        {
        ...
        public void log(string message)
        {
            if (!initzalized)
            {
                MessageBox.Show("Please initzalize first", "KaliSpammer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Environment.Exit(0);
            }
        ...
        {
            MessageBox.Show(message, "KaliSpammer", MessageBoxButtons.OK, MessageBoxIcon.Error);
            Environment.Exit(0);
        }
            ...
            catch (WebException webex)
            {
                ...
                switch (response.StatusCode)
                {
                    case (HttpStatusCode)429: // client hit our rate limit
                        MessageBox.Show("You're connecting too fast to loader, slow down.", "KaliSpammer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Environment.Exit(0);
                        return "";
                    default: // site won't resolve. you should use keyauth.uk domain since it's not blocked by any ISPs
                        MessageBox.Show("Connection failure. Please try again, or contact us for help.", "KaliSpammer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Environment.Exit(0);
                        return "";
                }
            }
    ...
    public static class encryption
    {
        ...
        public static byte[] str_to_byte_arr(string hex)
        {
            ...
            {
                MessageBox.Show("The session has ended, open program again.", "KaliSpammer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Environment.Exit(0);
                return null;
            }
        }
